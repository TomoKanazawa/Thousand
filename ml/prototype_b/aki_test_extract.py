"""Two-pass extract-then-diagnose variant.

Pass 1 — Extraction: read the chart, extract ALL notable findings
(abnormal labs with values, abnormal vitals, key history points,
medication clues, imaging findings). Output is a structured findings list.

Pass 2 — Diagnosis: given ONLY the extracted findings list (no chart),
produce a ranked top-30 differential. By separating noticing from
synthesizing, we hope the LLM doesn't get anchored on the chief complaint
and surfaces secondary findings (like AKI from elevated creatinine) more
reliably.

Cost: 2× API calls per case (~$0.08 each).
"""
...
EXTRACT_PROMPT = """You are a careful clinician reviewing a patient chart at the cutoff time.

Your task: extract EVERY notable finding from this chart. Be exhaustive, not selective. Include:
- All abnormal labs (with value, unit, reference range, and trend if visible)
- All abnormal vital signs
- Significant history elements (PMH, surgical hx, social hx)
- Active and home medications worth noting (especially those that signal disease or risk)
- Imaging findings if reported
- Microbiology findings if reported
- Notable physical exam findings
- Any other clinically meaningful data point

Be comprehensive. Do not filter for what you think matters most — list everything notable. A finding does not need to be life-threatening to be listed.

Return STRICT JSON only:

{
  "findings": [
    "Creatinine 1.8 mg/dL (ref 0.6-1.2), up from baseline 1.0 over 48h",
    "BP 88/52 on admission, MAP 64",
    "Lactate 4.2 mmol/L",
    "PMH includes type 2 diabetes, hypertension, CKD stage 3",
    "On lisinopril, metformin, atorvastatin at home",
    "CXR: bilateral lower lobe opacities consistent with pneumonia",
    ...
  ]
}
...
Below is a comprehensive list of notable findings extracted from the patient's chart at the cutoff time.

Your task: produce a ranked list of the TOP 30 most likely diagnoses that this patient has (or will be found to have) during this hospitalization. Synthesize related findings into specific clinical diagnoses.

Return STRICT JSON only, no prose:

{
  "differential": [
    {"rank": 1, "diagnosis": "...", "reasoning": "one short sentence citing which findings support this"},
...
def run_ddx(client, model, chart):
    # Pass 1: extract findings from chart
    ext_prompt = EXTRACT_PROMPT.replace("{CHART}", chart)
    def ext_call():
        return client.messages.create(model=model, max_tokens=8000,
                                       messages=[{"role": "user", "content": ext_prompt}])
    ext_resp = call_with_retry(client, ext_call)
    ext_parsed = extract_json(ext_resp.content[0].text)
    findings = ext_parsed.get("findings", [])
    findings_text = "\n".join(f"- {f}" for f in findings)

    # Pass 2: diagnose from findings (no chart)
    ddx_prompt = DDX_PROMPT.replace("{FINDINGS}", findings_text)
    def ddx_call():
        return client.messages.create(model=model, max_tokens=8000,
                                       messages=[{"role": "user", "content": ddx_prompt}])
    ddx_resp = call_with_retry(client, ddx_call)
    ddx_parsed = extract_json(ddx_resp.content[0].text)
    return ddx_parsed.get("differential", []), {
        "input_tokens": ext_resp.usage.input_tokens + ddx_resp.usage.input_tokens,
        "output_tokens": ext_resp.usage.output_tokens + ddx_resp.usage.output_tokens,
    }
...
def main():
    ...
    out_path = ROOT / f"results_aki_{args.model}_extract.json"
