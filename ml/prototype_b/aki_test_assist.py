"""Assist-mode variant of aki_test.py.

Models the production workflow: the LLM is told what's already documented
(PMH, admission reason) and asked to surface NEW acute conditions the
treating team may have missed. AKI is never mentioned in the prompt.

This is the methodologically cleanest test — no priming, no re-listing
of known chronic disease, focus on what's actually missed.

Outputs to results_aki_{model}_assist.json.
"""
...
DDX_PROMPT = """You are a senior internal medicine physician reviewing a patient chart at the cutoff time.

The chart includes the patient's past medical history (PMH), admitting reason / chief complaint, exam, labs, imaging, and medications.

Your task: produce a ranked list of all ACUTE conditions present or developing during THIS hospitalization. Rank by likelihood given the workup data.

Important:
- Skip the patient's chronic baseline conditions (the ones the chart presents as long-standing history). Those are already documented; do not re-list them.
- Do include acute presentations of chronic conditions if the workup shows new decompensation or acute progression.
- Be specific (e.g., "septic shock due to UTI" not "infection"). Use medical terminology that maps to ICD codes.

Return STRICT JSON only:

{
  "acute_diagnoses": [
    {"rank": 1, "diagnosis": "...", "reasoning": "one short sentence citing specific chart data"},
...
def run_ddx(client, model, chart):
    ...
    return parsed.get("acute_diagnoses", parsed.get("new_acute_diagnoses", parsed.get("differential", []))), {
...
def main():
    ...
    for case_dir in case_dirs:
        ...
        for cutoff in args.cutoffs:
            ...
            rank_15, dx_text_15 = detects_aki(ddx, k=15)
            ...
            results[cutoff][hadm_id] = {
                ...
                "aki_rank": rank_15,
                "aki_dx_text": dx_text_15,
                "hit@5": rank_5 is not None,
                "hit@15": rank_15 is not None,
            }

            verdict = "✓ caught" if rank_15 else "✗ missed"
            print(f"  {cutoff:<12s} {verdict}  rank={rank_15}  dx={dx_text_15[:60] if dx_text_15 else '—'}")

    # Aggregate
    print(f"\n=== AGGREGATE (model: {args.model}) ===")
    print(f"{'Cutoff':<15s}  {'hit@5':>15s}  {'hit@15':>15s}")
    for cutoff in args.cutoffs:
        ...
        n = len(rows)
        print(f"  {cutoff:<13s}  {h5}/{n} ({h5/n:.0%})        {h15}/{n} ({h15/n:.0%})")
    ...
    for cutoff in args.cutoffs:
        for sev in ["severe", "moderate", "mild"]:
            ...
            if not sev_rows:
                continue
            h = sum(r["hit@15"] for r in sev_rows)
            n = len(sev_rows)
            print(f"  {cutoff:<13s}  {sev:<10s}  {h}/{n} ({h/n:.0%})")

    print(f"\nTokens: in={in_total:,}  out={out_total:,}")
    out_path = ROOT / f"results_aki_{args.model}_assist.json"
