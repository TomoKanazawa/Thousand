"""Test whether the LLM detects AKI on hospital-undocumented cases.

Setup:
  - Input: case directory (one per hadm_id) produced by stitch_case.py
  - For each case at admit + plus24h cutoffs, ask the LLM for top-15 DDx
  - Score: did any candidate mention AKI / acute renal failure / etc.?

The candidate cases come from kdigo_aki_finder.py's selected_admissions_aki.json
— admissions that meet KDIGO criteria but had no AKI documentation in either
ICD codes or discharge summary text. These are the "truly never diagnosed"
test cases.

If the LLM flags AKI in these admit/24h charts, it has identified a clinically
real condition the hospital missed.
"""
...
def main():
    ...
    out_path = ROOT / f"results_aki_{args.model}.json"
